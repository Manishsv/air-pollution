...
def ingest_roads(city_id: str, bbox: dict, *, force: bool = False) -> int:
    ...
    """
    from urban_platform.h3_knowledge.ingestor import _check_interval, DEFAULT_H3_RES
    from urban_platform.h3_knowledge.writer import (
        write_signals, upsert_metadata, record_ingest,
    )
    from urban_platform.h3_knowledge.geo_agg import (
        aggregate_lines_to_h3, cells_for_bbox, cell_area_km2,
    )
    from urban_platform.connectors.geospatial.overpass_bbox import (
    ...
    written = write_signals(signal_rows, city_id=city_id, domain="roads", source="osm")
