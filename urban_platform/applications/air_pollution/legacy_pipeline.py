...
from urban_platform.applications.air_pollution import (
    aq_data,
...
from urban_platform.connectors.geospatial.osmnx_layers import download_osm_features
from urban_platform.common.data_audit import audit_data_coverage, print_audit_summary
from urban_platform.common.provenance import dataset_provenance_summary
from urban_platform.applications.air_pollution.folium_maps import (
...
from urban_platform.connectors.weather.hourly_archive import generate_synthetic_weather

# Platform layers (introduced incrementally)
from urban_platform.common import cache as cache_mod
...
def run_pipeline(
    ...
    *,
    ...
    if step == "sensor-siting":
        ...
        from urban_platform.applications.air_pollution.sensor_siting import run_sensor_siting
    ...
    for dtype in ["roads", "buildings", "landuse", "pois"]:
        ...
        if _cache_ok(cfg, osm_cache, refresh_scope=refresh_scope, artifact=dtype):
            ...
            for k, gdf in osm_outputs.items():
                ...
                cache_mod.save_cached_geodata(
                    gdf.to_crs("EPSG:4326") if gdf.crs and gdf.crs.to_string() != "EPSG:4326" else gdf,
                    path_k,
                )
    ...
    try:
        ...
        with open(reliability_path, "w", encoding="utf-8") as f:
            _json.dump(
                sanitize_for_json(reliability_df.to_dict(orient="records")),
                f,
                indent=2,
                default=str,
                allow_nan=False,
    ...
    # AQ panel (cached for backward compatibility)
    ...
    try:
        if observation_table is not None and not observation_table.empty and "source_reliability_status" in observation_table.columns:
    ...
    # Decision packets (human review artifact)
    feature_store_path = processed_dir / "feature_store.parquet"
    ...
    # Persist decision-support events (event_store)
    try:
        event_df = build_event_store(decision_packets=packets if isinstance(packets, list) else [], reliability_df=reliability_df)
