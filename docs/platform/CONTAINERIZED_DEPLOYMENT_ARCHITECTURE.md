...
**Related:** single-image quickstart and Docker usage are in [`docs/DOCKER_DEPLOYMENT.md`](DOCKER_DEPLOYMENT.md) (**Level 1**). A minimal multi-container **pilot-runtime Compose** path (Core API + dashboard, shared volumes) is in [`docs/DOCKER_COMPOSE_PILOT_RUNTIME.md`](DOCKER_COMPOSE_PILOT_RUNTIME.md). This document explains **why** Level 1 is a stepping stone and **what** comes next.
