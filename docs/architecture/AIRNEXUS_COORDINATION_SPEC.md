# AirNexus — Urban Coordination and Accountability Layer

## 1. Purpose

AirNexus is a standalone open-source service that provides entity resolution,
...
**Repo:** `airnexus` (separate repository under the AirOS organisation)
...
- AirNexus must work in non-DIGIT3 deployments (city platforms, custom stacks)
...
```
┌────────────────────────────────────────────────┐
│  AirNexus service  (:8300)                     │
│                                                │
│  REST API (FastAPI)                            │
│    /airnexus/v1/entities                       │
│    /airnexus/v1/links                          │
│    /airnexus/v1/events                         │
│    /airnexus/v1/traces                         │
│    /airnexus/v1/cases/{id}                     │
│                                                │
...
AirNexus is stateful but not authoritative for entity data — it stores
...
```
GET /airnexus/v1/cases/AIROS-BLR-2026-000042
X-Tenant-ID: {city_code}
...
```
GET /airnexus/v1/cases?rootEntityType=airos.ward&rootEntityId=BBMP_WARD_007
X-Tenant-ID: {city_code}
...
AirNexus calls are made **in parallel** with DIGIT3 calls — they are
non-blocking from the perspective of the decision packet pipeline. If AirNexus
is unavailable, decision generation continues; traces are queued and replayed
when AirNexus recovers.
...
airos-pipeline emits packet
  ...
  │
  └─→ AirNexus [async, non-blocking]:
        POST /airnexus/v1/events   (DECISION_GENERATED)
        POST /airnexus/v1/links    (ward → decision, decision → sensor)
        POST /airnexus/v1/traces   (step: generate, receiptHash from Governance)
```
...
AirOS webhook handler receives /airos/v1/webhooks/decision-updated
  ...
  │
  └─→ AirNexus [async]:
        POST /airnexus/v1/events  (DECISION_ACTIONED / DECISION_RESOLVED)
        POST /airnexus/v1/traces  (step: action, actor: officer-uuid)
```
...
When a decision is contested or audited, AirNexus provides the reconstruction
payload in a single query. The Reconstruction Engine (GCP component) calls:

```
GET /airnexus/v1/cases/{packetId}
```
...
| GCP requirement | Source in AirNexus response |
|----------------|-----------------------------|
...
AirNexus runs as a standalone container alongside AirOS services.
...
│  airos-dispatcher                                   │
│  airos-dashboard                                    │
│  airnexus (:8300)   ← new service                  │
│    PostgreSQL (airnexus schema, shared PG instance) │
...
```yaml
airnexus:
  image: airnexus:latest
  ports: ["8300:8300"]
  environment:
    DATABASE_URL: postgresql://pg/airnexus
    SQLITE_PATH: /data/airnexus.db
...
In production, AirNexus validates DIGIT3 Keycloak JWTs on the `X-Tenant-ID`
...
AirNexus is designed to work with any system that uses canonical string IDs.
It makes no assumptions about DIGIT3 — entity IDs are just strings.

A non-DIGIT3 deployment (e.g. city with a custom platform) can use AirNexus
...
platforms, future modules), shared event + graph layer in the middle (AirNexus),
