...
All three tabs import defensively from urban_platform.h3_knowledge.data_quality
...
from review_dashboard.ui_shell import render_context_metrics, render_section_title
from urban_platform.city_config import CITIES as _CITY_REGISTRY, get_centre, PANEL_CITIES
...
try:
    from urban_platform.h3_knowledge.data_quality import (
...
def _render_coverage_map(city_id: str) -> None:
    ...
    if not _DQ_AVAILABLE:
        st.info(
            "Module `urban_platform.h3_knowledge.data_quality` is not yet available. "
