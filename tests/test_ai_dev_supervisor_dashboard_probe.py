...
from airos.network.cli.ai_dev_supervisor.dashboard_probe import probe_dashboard
...
def test_dashboard_probe_matches_expected_labels() -> None:
    ...
    with patch("airos.network.cli.ai_dev_supervisor.dashboard_probe.requests.get", return_value=resp):
...
def test_dashboard_probe_unreachable_is_non_blocking() -> None:
    with patch(
        "airos.network.cli.ai_dev_supervisor.dashboard_probe.requests.get",
