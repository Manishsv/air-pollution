...
# Support `python airos/network/cli/airos_cli.py ...` (repo root may not be on sys.path yet).
_CLI_FILE = Path(__file__).resolve()
_REPO_ROOT_FOR_IMPORTS = _CLI_FILE.parents[3]
if str(_REPO_ROOT_FOR_IMPORTS) not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT_FOR_IMPORTS))

from airos.network.cli.deployment_runner.validate_deployment import ValidationSummary, validate_deployment

from airos.os import sdk as air_sdk
...
def _plan_supervisor(repo_root: Path, *, domain: str | None, run_conformance: bool) -> ExecPlan:
    argv = [
        sys.executable,
        "airos/network/cli/ai_dev_supervisor/run_review.py",
    ]
    if domain:
        argv += ["--domain", domain]
    if run_conformance:
...
def _plan_deployment_run(repo_root: Path, deployment: str, *, store_dir: str | None = None) -> ExecPlan:
    argv: list[str] = [
        sys.executable,
        "airos/network/cli/deployment_runner/run_deployment.py",
        "--deployment",
        deployment,
    ]
    if store_dir:
...
def _examples_describe(repo_root: Path, name: str) -> int:
    ...
    if not example_dir.is_dir():
        ...
        print("Run: python airos/network/cli/airos_cli.py examples list", file=sys.stderr)
        return 1
    ...
    print(f"- validate: {sys.executable} airos/network/cli/airos_cli.py deployment validate {rel}")
    print(f"- run: {sys.executable} airos/network/cli/airos_cli.py deployment run {rel}")
    return 0
...
def _health_local(repo_root: Path) -> int:
    ...
    try:
        from airos.os.sdk.apps import list_app_descriptors
    ...
    try:
        from airos.os.sdk.adapters import list_provider_adapter_descriptors
    ...
    try:
        from airos.os.sdk.catalogs import list_reference_catalogs
    ...
    try:
        from airos.os.sdk.deployments import list_deployment_profiles
    ...
    try:
        from airos.os.deployments.builder_registry import list_builders
...
def _apps_explain(repo_root: Path, app_id: str) -> int:
    ...
    from airos.os.deployments.builder_registry import has_builder  # noqa: WPS433
    ...
    for it in ex:
        ...
        if path:
            print(f"  validate: python airos/network/cli/airos_cli.py deployment validate {path}")
            print(f"  run:      python airos/network/cli/airos_cli.py deployment run {path}")
    ...
    print(f"- Inspect the app descriptor: python airos/network/cli/airos_cli.py apps show {aid}")
    for ck in inputs[:2] if isinstance(inputs, list) else []:
        if isinstance(ck, str) and ck.strip():
            print(f"- Inspect input contract schema: python airos/network/cli/airos_cli.py contracts show {ck}")
            break
    if aid == "program_reporting_review":
        print("- Validate a known fixture:")
        print(
            "  python airos/network/cli/airos_cli.py fixtures validate consumer_city_program_submission "
            "specifications/examples/program_reporting/city_program_submission.sample.json"
        )
        print("- Run the program reporting example:")
        print("  python airos/network/cli/airos_cli.py deployment run deployments/examples/program_reporting_state_demo")
    if aid == "flood_risk_review":
        print("- Run the flood example:")
        print("  python airos/network/cli/airos_cli.py deployment run deployments/examples/flood_local_demo")
...
def _deployment_validate(repo_root: Path, deployment: str) -> int:
    validator = repo_root / "airos" / "network" / "cli" / "deployment_runner" / "validate_deployment.py"
    if not validator.is_file():
        print("Deployment validation is not implemented yet.")
        print("Recommended next task: add airos/network/cli/deployment_runner/validate_deployment.py")
        return 2
    ...
    *,
    ...
    if from_example:
        ...
        try:
            ...
            pass
        print("Next steps:")
        print(f"1) Validate config: {sys.executable} airos/network/cli/airos_cli.py deployment validate {rel}")
        print(f"2) Run deployment: {sys.executable} airos/network/cli/airos_cli.py deployment run {rel}")
        print(f"3) Inspect outputs under data/outputs/deployments/{deployment_id}/ after a successful run.")
        print(f"4) Run governance checks: {sys.executable} airos/network/cli/airos_cli.py review --run-conformance")
    ...
    (out_path / "README.md").write_text(
        "\n".join(
            [
                ...
                "  - `python airos/network/cli/airos_cli.py deployment validate <this-folder>`",
                "- Run conformance:",
    ...
    print(f"2) Validate config only: {sys.executable} airos/network/cli/airos_cli.py deployment validate {rel}")
    print(f"3) When valid and configured for a supported demo, run: {sys.executable} airos/network/cli/airos_cli.py deployment run <path>")
    print(f"4) Inspect outputs under data/outputs/deployments/<deployment_id>/ after a successful run.")
    print(f"5) Run governance checks: {sys.executable} airos/network/cli/airos_cli.py review --run-conformance")
    ...
    (out_path / "README.md").write_text(
        "\n".join(
            [
                ...
                "",
                "This folder was generated by `python airos/network/cli/airos_cli.py apps scaffold ...`.",
                "",
                "## Important",
                "",
...
def _apps_validate(repo_root: Path, app_path: str) -> int:
    ...
    if descriptor is not None:
        ...
        try:
            from airos.network.api.app_descriptors import _load_descriptor_schema_validator  # noqa: WPS433
        ...
        if not ymls:
            ...
            print("- (none)")
        try:
            from airos.network.cli.deployment_runner.validate_deployment import _check_no_secrets  # noqa: WPS433
        except Exception:
...
def _catalog_list(repo_root: Path, *, catalog_dir: str | None) -> int:
    ...
    if not isinstance(apps, dict) or not apps:
        print("Catalog is empty. Add a package with: python airos/network/cli/airos_cli.py catalog add-package <zip>")
        return 0
...
_DEPLOYMENT_INIT_EPILOG = """Example:
  python airos/network/cli/airos_cli.py deployment init --deployment-id my_city --deployment-name "My City" \\
